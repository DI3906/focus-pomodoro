...
const taskTest = [
  {
    id: 1,
    description: "Ajustar en un contenedor, el icono al inicio de este, el parrafo debera ocupar 2/3 aprox de todo y al final si esta completado se debera mostrar un icono para eliminar la tarea",
    completed: false
  },
  {
    id: 2,
    description: "Si funciona pero el problema es cuando son pocas palabras y no logran ocupar todo el espacio",
    completed: true
  },
  {
    id: 3,
    ...
    <div className="w-full h-full rounded-2xl p-6 flex flex-col justify-between todo">
      <ul>
        {taskTest === undefined || taskTest.length == 0 ? (
          ...
          taskTest.map((task) =>
            <Task id={task.id} detail={task.description} isCompleted={task.completed} />
          )
        )}
      </ul>
